...
--과제: kochar 에게 보고하는 사원들의 이름,월급 을 조회하라.
select last_name,salary
from employees
...
--과제: 이름에u가 포함된 사원이 있는 부서에서 일하는 사원들의
--      사번,이름을 조회하라.
select employee_id,last_name
from employes
...
--과제: 1700번 지역에 위치한 부서에서 일하는 사원들의
--      이름,직업,부서번호를 조회하라.
...
--any 는 in과 뜻이같다.다른연산자와 결합해서 쓸수있다.
select employee_id,last_name
from employees
...
--과제: 회사 평균 월급보다, 그리고 모든 프로그래머보다 월급을 더받는
-- 사원들의 이름,직업,월급을 조회하라.
select last_name,job_id,salary
from employees
where salary > (select avg(salary)
