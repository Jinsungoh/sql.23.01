...
select last_name,to_char(hire_date,'YYYY.MM.DD') hire_date,
            to_char(next_day(add_months(hire_date,3),'monday'),'YYYY.MM.DD') review_data   
from employees;
...
from dual;
--nvl 은 첫번쨰null값을 없애준다.같은 데이터 타입을써야한다.
select nvl(null, 0)
from dual;
...
from employees;
--기준값은 기준값끼리 리턴값은 리턴값끼리 타입이 같아야한다.
