--single function,공통점은 로우한개가 들어간다.
desc dual
select * from dual;
...
select last_name
from employees
where lower (last_name) = 'higgins';--lower function 값을 비교한다.
...
--과제: 위 query에서, where절을 like로 refactoring 하라.
select employee_id, concat(first_name, last_name) name,
            job_id, length(last_name), instr(last_name, 'a')"Contains 'a'?"
from employees
